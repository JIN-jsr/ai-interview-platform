...
每次提交回答后，系统先通过本地规则生成轻量即时分析，再继续后续提问。即时分析属于单题级训练反馈，不直接等同于最终五维评分，也不会改变既有追问调度和最终评分权重。
...
每次提交回答后，系统先通过本地规则生成轻量即时分析，展示已覆盖要点、主要遗漏、风险表述和改进方向。启用 LLM 辅助反馈润色时，模型只优化文字表达，不修改覆盖率、临时表现、最终分数或五维权重。活跃面试期间不会展示完整参考答案，完整五维评分仍在完整面试结束后统一生成。
...
界面保持黑白灰风格，首页提供“系统介绍”和“模拟面试”入口。系统介绍页说明平台定位、工作流程、技术特色和使用边界；模拟面试页聚焦当前问题、问题详情、回答提交、答后分析、报告跳转和状态展示。即时分析采用可折叠卡片，默认展示摘要，详细依据按需展开，并与修改最近回答重新提交功能形成“回答—反馈—修改—再验证”的训练闭环；侧边栏提供“答后即时分析”和“LLM 辅助反馈润色”两个轻量开关。面试结束时，“导出即时分析”和“查看报告”并排展示，前者用于逐题复盘，后者进入正式五维评分报告。
