...
```text
简历输入/上传 -> 简历解析 -> 候选人画像 -> RAG 知识检索
-> LLM + RAG 协同出题 -> 用户回答 -> 即时分析 -> 动态追问
-> 逐题复盘导出 -> 最终五维评分报告 -> 多格式导出 -> 能力成长复盘
```
...
- 支持答后即时分析，展示回答亮点、主要遗漏、风险表述和改进方向。
- 支持 LLM 可选反馈润色，qwen3.7-plus 只优化文字表达，不修改评分数据。
- 支持本地规则反馈 fallback，模型不可用时仍可生成训练反馈。
- 支持修改并重新提交最近回答，重新分析会替换旧记录。
- 支持即时分析 Markdown 导出，完整面试结束后可用于逐题复盘。
...
- LLM 辅助边界清晰：LLM 只优化反馈表达，不直接修改覆盖率、临时分或最终五维评分。
...
用户输入或上传简历
  ...
  -> 逐题复盘导出
  -> 最终五维评分报告
  -> Markdown / JSON / PNG 导出
  -> 历史记录与能力成长曲线
```

即时分析用于单题训练反馈；最终五维评分在完整面试结束后统一生成。若开启“LLM 辅助反馈润色”且 LLM 配置可用，系统会在本地规则分析后润色每题反馈和总体回答分析；LLM 只优化反馈文字表达，不修改覆盖率、临时表现、最终分数或评分权重，超时或关闭时自动回退本地规则。
